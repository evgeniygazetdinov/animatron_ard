        ...
        while True:
            ...
            for p in self.exist_posistion:
                if p[0]== self.timer_seconds:
                    ...
                    #rebuild after
                    l_e_nine_pin.write(p[1])
                    # r_e_eight_pin.write(p[2])
                    # sh_r_seven_pin.write(p[3])
                    # r_h_three_pin.write(p[4])
                    # l_h_six_pin.write(p[5])
                    # l_l_four_pin.write(p[6])
                    # r_l_five_pin.write(p[7])
                    # res_1_ten_pin.write(p[8])
                    # res_2_eleven_pin.write(p[9])
                    False
    def some_play(self):
