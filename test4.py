...
class SERVO_MAN(Variables,Player,Default_position,
    ...
    def calculate_scale(self,first_p,time_begin,time_over):
        ...
        print(time_begin)
        print("HUEC"+str(time_over))
        execute = self.generateNumber(time_begin,time_over,min_interval,fi)
        execute_with_servo = self.servo_on_min_interval(execute,self.left_eye.get(),self.right_e.get(),
                                   self.right_sholder.get(),self.right_hand.get(),
                                   self.left_hand.get(),self.left_leg.get(),
                                   self.right_leg.get(),self.reserved_1.get(),
                                   self.reserved_2.get(),0,2,4,6,8,10,12,14,16)
